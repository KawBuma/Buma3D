...
class B3D_API DescriptorSet0Vk : public IDeviceChildVk<IDescriptorSet0>, public util::details::NEW_DELETE_OVERRIDE
{
    friend class DescriptorPool0Vk;
protected:
    B3D_APIENTRY DescriptorSet0Vk();
    DescriptorSet0Vk(const DescriptorSet0Vk&) = delete;
    B3D_APIENTRY ~DescriptorSet0Vk();

private:
    BMRESULT B3D_APIENTRY Init(DescriptorPool0Vk* _pool, RootSignatureVk* _signature);
    BMRESULT B3D_APIENTRY AllocateDescriptors();
    void B3D_APIENTRY Uninit();

public:
    static BMRESULT
        B3D_APIENTRY Create(DescriptorPool0Vk* _pool, RootSignatureVk* _signature, DescriptorSet0Vk** _dst);
    ...
    BMRESULT
        B3D_APIENTRY CopyDescriptorSet(IDescriptorSet0* _src) override;
