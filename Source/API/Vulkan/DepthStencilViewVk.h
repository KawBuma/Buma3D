...
class B3D_API DepthStencilViewVk : public IDeviceChildVk<IDepthStencilView>, public IViewVk, public util::details::NEW_DELETE_OVERRIDE
{
protected:
    B3D_APIENTRY DepthStencilViewVk();
    DepthStencilViewVk(const DepthStencilViewVk&) = delete;
    B3D_APIENTRY ~DepthStencilViewVk();

private:
    BMRESULT B3D_APIENTRY Init(DeviceVk* _device, IResource* _resource, const DEPTH_STENCIL_VIEW_DESC& _desc);
    void B3D_APIENTRY CopyDesc(const DEPTH_STENCIL_VIEW_DESC& _desc);
    BMRESULT B3D_APIENTRY ValidateTextureRTV();
    BMRESULT B3D_APIENTRY PrepareImageLayout();
    BMRESULT B3D_APIENTRY InitAsImageView();
    void B3D_APIENTRY Uninit();

public:
    static BMRESULT
        B3D_APIENTRY Create(DeviceVk* _device, IResource* _resource, const DEPTH_STENCIL_VIEW_DESC& _desc, DepthStencilViewVk** _dst);
