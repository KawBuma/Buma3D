...
class B3D_API DescriptorSet0D3D12 : public IDeviceChildD3D12<IDescriptorSet0>, public util::details::NEW_DELETE_OVERRIDE
{
    friend class DescriptorPool0D3D12;
...
protected:
    B3D_APIENTRY DescriptorSet0D3D12();
    DescriptorSet0D3D12(const DescriptorSet0D3D12&) = delete;
    B3D_APIENTRY ~DescriptorSet0D3D12();

private:
    BMRESULT B3D_APIENTRY Init(DescriptorPool0D3D12* _pool, RootSignatureD3D12* _signature);
...
public:
    static BMRESULT
        B3D_APIENTRY Create(DescriptorPool0D3D12* _pool, RootSignatureD3D12* _signature, DescriptorSet0D3D12** _dst);
