...
uint32_t
B3D_APIENTRY DescriptorPoolD3D12::GetCbvSrvUavCountsInPoolSizes()
{
    uint32_t result = 0;
...
uint32_t
B3D_APIENTRY DescriptorPoolD3D12::GetSamplerCountsInPoolSizes()
{
    uint32_t result = 0;
    for (uint32_t i = 0; i < desc.num_pool_sizes; i++)
    {
...
BMRESULT
B3D_APIENTRY DescriptorPoolD3D12::CreateDescriptorHeaps(uint32_t _num_descs, uint32_t _num_sampler_descs)
{
...
BMRESULT
B3D_APIENTRY DescriptorPoolD3D12::AllocateDescriptorSet(IRootSignature* _root_signature, IDescriptorSet** _dst)
{
    std::lock_guard lock(allocation_mutex);
    ...
    B3D_RET_IF_FAILED(DescriptorSetD3D12::Create(this, rs, &ptr));
...
B3D_APIENTRY DescriptorPoolD3D12::GetDescHeapAllocators()
{
    return dh_allocators;
}

const util::DyArray<ID3D12DescriptorHeap*>&
B3D_APIENTRY DescriptorPoolD3D12::GetD3D12DescriptorHeaps() const
{
    return desc_heaps12;
}
...
BMRESULT
B3D_APIENTRY DescriptorPoolD3D12::AllocateDescriptors(DescriptorSetD3D12* _set)
{
    // OPTIMIZE: DescriptorSetD3D12::AllocateDescriptors()、ないしディスクリプタ系全般をリファクタ
...
void
B3D_APIENTRY DescriptorPoolD3D12::FreeDescriptors(DescriptorSetD3D12* _set)
{
    auto&& ps          = _set->signature->GetPoolSizes();
